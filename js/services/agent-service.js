...
class AgentService {
    ...
    initialize(campaignAgents = []) {
        console.log('👥 Initializing AgentService with', campaignAgents.length, 'agents');
        ...
        console.log(`✅ AgentService initialized: ${this.activeAgents.length} active, ${this.availableAgents.length} available`);
    }
    ...
    killAgent(agentId, killer = null) {
        ...
        if (!agent.alive) {
            console.warn(`⚠️ Agent ${agent.name} is already dead`);
            return false;
        }
        ...
        console.log(`💀 ${agent.name} has fallen${killer ? ` to ${killer}` : ''}`);
        return true;
    }
    ...
    reviveAgent(agentId, cost = 5000) {
        ...
        if (!agent) {
            console.error(`❌ Fallen agent not found: ${agentId}`);
            return false;
        }

        // Check resources
        if (this.resourceService && !this.resourceService.canAfford('credits', cost)) {
            console.warn(`⚠️ Cannot afford revival (cost: ${cost})`);
            return false;
        }
        ...
        console.log(`✨ ${agent.name} has been revived for ${cost} credits`);
        return true;
    }
    ...
    selectAgentsForMission(agentIds) {
        ...
        for (const id of agentIds) {
            ...
            if (agent && agent.alive && agent.hired) {
                ...
                console.warn(`⚠️ Cannot select agent: ${id}`);
            }
        }
        ...
        console.log(`👥 Selected ${this.selectedAgents.length} agents for mission`);
        return this.selectedAgents;
    }
    ...
    healAgent(agentId, amount) {
        ...
        console.log(`💚 Healed ${agent.name}: ${oldHealth} → ${agent.health}`);
        return true;
    }
    ...
    damageAgent(agentId, amount, source = null) {
        ...
        console.log(`💔 ${agent.name} damaged: ${oldHealth} → ${agent.health}${source ? ` by ${source}` : ''}`);
    ...
    notifyListeners(eventType, data) {
        ...
        if (this.listeners[eventType]) {
            this.listeners[eventType].forEach(callback => {
                try {
                    ...
                    console.error(`Error in agent listener (${eventType}):`, e);
                }
            });
        }
        ...
        this.listeners.any.forEach(callback => {
            try {
                ...
                console.error('Error in global agent listener:', e);
            }
        });
    }
    ...
    importState(state) {
        ...
        console.log('👥 AgentService state imported');
    }
    ...
    reset() {
        ...
        this.agentByName.clear();
        this.nextAgentId = 1;
        console.log('👥 AgentService reset');
    }
}
