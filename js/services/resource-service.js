...
class ResourceService {
    ...
    initialize(initialState = {}) {
        ...
        console.log('💰 ResourceService initialized:', this.resources);
    }
    ...
    set(resourceType, value, reason = 'direct set') {
        ...
        console.log(`💰 ${resourceType}: ${oldValue} → ${newValue} (${reason})`);
        return true;
    }
    ...
    spend(resourceType, amount, reason = 'purchase') {
        ...
        if (currentValue < amount) {
            console.warn(`⚠️ Cannot afford ${amount} ${resourceType} (have: ${currentValue})`);
            return false;
        }
        ...
        for (const change of changes) {
            ...
            if (operation === 'spend' || (operation === 'add' && amount < 0)) {
                ...
                if (!this.canAfford(resource, spendAmount)) {
                    console.warn(`⚠️ Transaction failed: Cannot afford ${spendAmount} ${resource}`);
                    return false;
                }
            }
    ...
    notifyListeners(resourceType, oldValue, newValue) {
        ...
        if (this.listeners[resourceType]) {
            this.listeners[resourceType].forEach(callback => {
                try {
                    ...
                    console.error('Error in resource listener:', e);
                }
            });
        }
        ...
        this.listeners.any.forEach(callback => {
            try {
                ...
                console.error('Error in global resource listener:', e);
            }
        });
    }
    ...
    importState(state) {
        ...
        console.log('💰 ResourceService state imported:', this.resources);
    }
    ...
    reset() {
        ...
        this.history = [];
        console.log('💰 ResourceService reset to defaults');
    }
}
