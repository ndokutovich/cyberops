...
CyberOpsGame.prototype.loadMissionData = function(missionIndex) {
    ...
    if (!missionDef) {
        ...
        console.error('Campaign system is required. Check campaigns/main/ folder.');
        return null;
    }
    ...
    if (urlParams.get('test_mission') === 'true') {
        ...
        if (testMissionData) {
            try {
                ...
                console.log('🧪 Loading test mission from editor:', testMission.name);
                ...
                console.error('Failed to load test mission:', e);
            }
        }
    }
...
CyberOpsGame.prototype.generateMapWithObjects = function(mapDef) {
    console.log('🗺️ generateMapWithObjects called with mapDef:', mapDef);
    console.log('- useDeclarativeMap:', mapDef?.useDeclarativeMap);
    console.log('- generateMapFromDefinition exists:', typeof generateMapFromDefinition !== 'undefined');
    console.log('- current map exists:', !!this.map);
    console.log('- current map has tiles:', !!(this.map && this.map.tiles));

    // FIXED: Don't regenerate map if it already exists!
    if (this.map && this.map.tiles) {
        console.log(`📍 Map already generated (${this.map.width}x${this.map.height}), skipping regeneration`);
...
CyberOpsGame.prototype.loadCustomMapFromEditor = function(customData) {
    console.log('📝 Loading custom map from editor:', customData.name);
